...
const postParser = async (url) => {
  try {
    ...
    const seriesElem = $('[itemprop="position"][content="2"]').prev();
    const seriesurlElem = $('[itemprop="position"][content="2"]').parent();
    const datetimeElem = $('[property="article:published_time"]');
    const descriptionElem = $('em');
    ...
    } else if ($('.single-featured img').length > 0) {
      thumbnailElem = $('.single-featured img');
    }
    ...
    const srcset = srcsetText ? srcsetText.split(', ') : [];
    ...
    const post = {
      ...
      url,
      thumbnailUrl: thumbnailElem.attr('data-src') || 'https://dummyimage.com/640x360/000000/ffffff.png&text=No+Image',
